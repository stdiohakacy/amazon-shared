...
import {
  DocAuthOptionsInterface,
  DocDefaultOptionsInterface,
  DocGuardOptionsInterface,
  DocOfOptionsInterface,
  DocOptionsInterface,
  DocRequestFileOptionsInterface,
  DocRequestOptionsInterface,
  DocResponseFileOptionsInterface,
  DocResponseOptionsInterface,
...
import { ENUM_POLICY_STATUS_CODE_ERROR } from '../../policies/enums/policy.status-code.enum';
import { ENUM_AUTH_STATUS_CODE_ERROR } from '../../auth/enums/auth.status-code.enum';
import { ENUM_API_KEY_STATUS_CODE_ERROR } from '../../api-key/enums/api-key.status-code.enum';
import { ENUM_FILE_MIME } from '../../file/enums/file.enum';

export function DocDefault<T>(
  options: DocDefaultOptionsInterface<T>,
): MethodDecorator {
  const docs = [];
...
export function DocOneOf(
  ...
  ...documents: DocOfOptionsInterface[]
): MethodDecorator {
  const docs = [];
  const oneOf = [];
...
export function Doc(options?: DocOptionsInterface): MethodDecorator {
  return applyDecorators(
    ApiOperation({
...
export function DocRequest(options?: DocRequestOptionsInterface) {
...
export function DocRequestFile(options?: DocRequestFileOptionsInterface) {
  const docs: Array<ClassDecorator | MethodDecorator> = [];
...
export function DocGuard(options?: DocGuardOptionsInterface) {
  const oneOfForbidden: DocOfOptionsInterface[] = [];
...
export function DocAuth(options?: DocAuthOptionsInterface) {
  const docs: Array<ClassDecorator | MethodDecorator> = [];
  const oneOfUnauthorized: DocOfOptionsInterface[] = [];
...
export function DocResponse<T = void>(
  messagePath: string,
  options?: DocResponseOptionsInterface<T>,
): MethodDecorator {
  const docs: DocDefaultOptionsInterface = {
...
export function DocResponseFile(
  options?: DocResponseFileOptionsInterface,
): MethodDecorator {
